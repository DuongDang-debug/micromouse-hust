...
float speed_r;
  ...
  #define TURN_90_COUNTS    400        // calibrate, lucs ddaauf laf 830
...
// THÊM MỚI
bool prevTwoWallsSeen = false;
int bothWallStableCount = 0;

const float INTERSECTION_MIN_CM = 2.0f;   // đã đi ít nhất chừng này mới được phép thoát mode
const float INTERSECTION_MAX_CM = 10.0f;  // nếu đi quá xa mà chưa thấy lại 2 tường thì vẫn thoát cưỡng bức
const int BOTH_WALL_STABLE_NEED = 4;      // cần thấy lại 2 tường liên tiếp N chu kỳ

int prevEL_pid = 0;
int prevER_pid = 0;
bool pidEncReady = false;

const float KP_ENC_MAIN    = 3.0f;   // encoder giữ thẳng
const float KP_WALL_CENTER = 3.0f;   // 2 tường -> kéo về giữa nhẹ thôi
const float KP_WALL_ANGLE = 1.0f; // Cần tinh chỉnh thực tế trên xe

const float WALL_CENTER_LIM = 10.0f;  // giới hạn lỗi bám giữa để khỏi giật

const float KP_WALL_PAR    = 6.0f;   // giữ song song với 1 tường
const float WALL_PAR_LIM   = 10.0f;   // giới hạn lỗi song song
const float KP_WALL_PUSH   = 22.0f;  // đẩy ra nếu quá sát
const float WALL_MARGIN_CM = 2.0f;   // ngưỡng quá sát

// 1. THÊM 2 BIẾN NÀY LÊN TRÊN CÙNG (Bên ngoài các hàm)
float target_enc_diff = 0; 
bool in_intersection_mode = false;

// THÊM BIẾN NÀY LÊN ĐẦU FILE
const float K_LEAK = 0.1f; // Hệ số mốc mềm (0.01 đến 0.1). Càng nhỏ encoder giữ càng cứng.
...
void PID() {
  ...
  // THÊM LOGIC PHÁT HIỆN GÓC TƯỜNG Ở ĐÂY
  // d5 - refR5 vì d5 là khoảng cách thực tế, nếu ra ngã tư nó sẽ lớn hơn khoảng cách chuẩn
  bool leftJumping = (d1 - refL1) > OPEN_JUMP_CM;
  bool rightJumping = (d5 - refR5) > OPEN_JUMP_CM; 

  // Chỉ dùng tường nếu nó đang ổn định (không bị giật nhún)
  bool validLeft = leftSeen && !leftJumping;
  bool validRight = rightSeen && !rightJumping;
  bool validTwoWalls = validLeft && validRight;

  noInterrupts();
  long EL = encoder1_val; 
  long ER = encoder2_val;
  interrupts();

  long current_diff = EL - ER;

  // ===== KẾT HỢP ENCODER VÀ TƯỜNG =====
  if (validTwoWalls || validLeft || validRight) {
    // 1. KHI CÓ TƯỜNG: Áp dụng kỹ thuật "Mốc mềm"
    // Thay vì: target_enc_diff = current_diff; (làm triệt tiêu encoder)
    // Ta cho mốc từ từ đuổi theo current_diff.
    target_enc_diff += ((current_diff - target_enc_diff) * K_LEAK); 
    
    in_intersection_mode = false;
  } else {
    // 2. KHI MẤT TƯỜNG (NGÃ TƯ): Khóa chết mốc!
    if (!in_intersection_mode) {
      target_enc_diff = current_diff; // Chốt cứng mốc ĐÚNG 1 LẦN để qua ngã tư
      in_intersection_mode = true;
    }
  }

  // Tính sai số tích lũy từ mốc đã khóa
  float e_enc = (float)(current_diff - target_enc_diff);
  float u_enc = KP_ENC_MAIN * e_enc;

  // ===== Wall assist =====
  float u_wall = 0.0f;

  if (validTwoWalls) {
    // ===== 2 tường: giữ giữa nhẹ =====
    float e_center = (d1 - refL1) - (d5 - refR5);
    e_center = constrain(e_center, -WALL_CENTER_LIM, WALL_CENTER_LIM);
    float e_angle = (d2 - refL2) - (d4 - refR4);
    
    // Đổi tên thành u_center cho rõ ràng
    float u_center = (KP_WALL_CENTER * e_center) + (KP_WALL_ANGLE * e_angle);

    // THÊM: LOGIC ĐẨY KHẨN CẤP NGAY CẢ KHI CÓ 2 TƯỜNG
    float u_push = 0.0f;
    float tooCloseL = (refL1 - WALL_MARGIN_CM) - d1;
    float tooCloseR = (refR5 - WALL_MARGIN_CM) - d5;
    
    if (tooCloseL > 0.0f) {
      u_push += KP_WALL_PUSH * tooCloseL;  // QUAN TRỌNG: Phải là DẤU TRỪ
    }
    if (tooCloseR > 0.0f) {
      u_push -= KP_WALL_PUSH * tooCloseR;  // QUAN TRỌNG: Phải là DẤU CỘNG
    }

    // QUAN TRỌNG NHẤT: Bắt buộc phải cộng u_push vào u_wall
    u_wall = u_center + u_push; 
  }
  else if (validLeft) {
    // ===== Chỉ còn tường trái =====
    float e_dist = d1 - refL1;   
    float e_angle = d2 - refL2;  
    
    float e_track = e_dist + e_angle; 
    e_track = constrain(e_track, -WALL_PAR_LIM, WALL_PAR_LIM);

    float u_parallel = KP_WALL_PAR * e_track;

    float u_push = 0.0f;
    float tooCloseL = (refL1 - WALL_MARGIN_CM) - d1;
    if (tooCloseL > 0.0f) {
      u_push += KP_WALL_PUSH * tooCloseL;  
    }

    u_wall = u_parallel + u_push;
  }
  else if (validRight) {
    // ===== Chỉ còn tường phải =====
    float e_dist = refR5 - d5;   
    float e_angle = refR4 - d4;  
    ...
    float tooCloseR = (refR5 - WALL_MARGIN_CM) - d5;
    if (tooCloseR > 0.0f) {
      u_push -= KP_WALL_PUSH * tooCloseR;   
    }

    u_wall = u_parallel + u_push;
  }
  else {
    // ===== Không còn tường (Đang trong ngã tư) =====
    // Lúc này u_wall = 0, u_enc sẽ gánh toàn bộ trọng trách giữ đầu xe thẳng tắp!
    u_wall = 0.0f;
  }

  float correction = u_enc + u_wall;
  correction = constrain(correction, -60.0f, 60.0f);

  speed_l = constrain(SPEED_L - correction, -250.0f, 250.0f);
  speed_r = constrain(SPEED_R + correction, -250.0f, 250.0f);
}
...
bool motor_run_fast() {
  ...
  return true;
    ...
    // Nếu chưa đạt đích, tiếp tục di chuyển
      ...
      SPEED_R = SPEED_L;  // Điều chỉnh cân bằng tốc độ bánh trái và phải
...
void reset_PID_vars() {
  ...
  intersectionStartDist = 0.0f;
  encBiasAtIntersection = 0;
